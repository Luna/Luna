...
`('Node information from AST $line line', ({ line, pattern, rootExpr, documentation }) => {
  const ast = [...Ast.Ast.parseBlock(line).statements()][0]!
  ...
  expect(node?.outerExpr.isStatement() && node.outerExpr.documentationText()).toBe(documentation)
})

test.each(['## Documentation only'])("'%s' should not be a node", (line) => {
  const ast = Ast.Ast.parseStatement(line)
  const node = nodeFromAst(ast, false)
...
test.each([
  ...
  const ast = Ast.Ast.parseExpression(code)
